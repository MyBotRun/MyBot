...
Func getArmyTroopCount($bOpenArmyWindow = False, $bCloseArmyWindow = False, $test = False)
	...
	If $g_bDebugSetlogTrain Or $g_bDebugSetlog Then SetLog("Begin getArmyTroopCount:", $COLOR_DEBUG1)
	...
	If $g_bDebugSetlog Then SetDebugLog("$g_hHBitmap2 made", $COLOR_DEBUG)
	If _Sleep($DELAYCHECKARMYCAMP5) Then Return
	If $g_bDebugSetlogTrain Then SetLog("Calling " & $g_sMBRLib & "/searchIdentifyTroopTrained ", $COLOR_DEBUG)
	...
	If $g_bDebugSetlogTrain Then
		...
		EndIf
	EndIf
	If $g_bDebugSetlogTrain Then SetLog("Start the Loop", $COLOR_DEBUG)
	...
	If IsArray($TroopTypeT) And $TroopTypeT[1] <> "" Then
		...
		For $i = 1 To $TroopTypeT[0]
			...
			If $g_bDebugSetlogTrain Then SetLog("$TroopTypeT[$i] split : " & $i, $COLOR_DEBUG)
