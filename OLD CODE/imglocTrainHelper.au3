...
Func QuickTrain($quickTrainOption, $bOpenAndClose = True, $forceDebug = False)
	...
	For $rt = 0 To $retry
		...
		$QuickTrainTabBtn = isButtonVisible("QuickTrainTabBtn", $g_sImgTrainQuickTrain, $QuickTrainTabArea)
	...
	For $rt = 0 To $retry
		...
		$CheckTabBtn = isButtonVisible("CheckTabBtn", $g_sImgTrainCheckTab, $QuickTrainTabArea) ;check same region for White Area when tab selected
		If _Sleep($g_iQuickTrainButtonRetryDelay) Then Return
		If $CheckTabBtn <> "" Then
	...
	Switch $quickTrainOption
		Case 0 ; Previous Army
			...
			$optBtn = isButtonVisible("Previous Army", $g_sImgTrainPrevArmy, $region)
			$selBtn = "Previous Army"
		Case 1 ; Army 1
			$region = "725,335,840,375"
			$optBtn = isButtonVisible("Army1", $g_sImgTrainArmy, $region)
			$selBtn = "Army1"
		Case 2 ; Army 2
			$region = "725,455,840,495"
			$optBtn = isButtonVisible("Army2", $g_sImgTrainArmy, $region)
			$selBtn = "Army2"
		Case 3 ; Army 3
			$region = "725,570,840,615"
			$optBtn = isButtonVisible("Army3", $g_sImgTrainArmy, $region)
			$selBtn = "Army3"
	EndSwitch
...
Func imglocTrainIfAvailable($nTroopEnum, $iQuantity, $imglocFoundArray)
	...
	If $g_bDebugSetlog Then SetDebugLog($sTroopName & " / " & $nTroopEnum & " training if available!", $COLOR_DEBUG)
	Local $aLineValue
	For $iFA = 0 To UBound($imglocFoundArray) - 1
		...
		If $aLineValue[0] = $sTroopName Then ; found troop/spell to click
			If $g_bDebugSetlog Then SetDebugLog($sTroopName & " / " & $nTroopEnum & " found. Checking availability!", $COLOR_DEBUG)
...
Func imglocFindAvailableToTrain($sTrainType)
	...
	Local $sDirectory = $g_sImgTrainArmyDir & $sTrainType & "\"
	If $g_bDebugSetlog Then SetDebugLog("imgloc Searching Regular Troops :  in " & $sCocDiamond & " using " & $sDirectory, $COLOR_INFO)
	Local $bForceCapture = True ; force CaptureScreen
	;aux data
	If $g_bDebugSetlog Then SetDebugLog("imgloc train search : " & $sTrainType, $COLOR_DEBUG)
