...
Func IsScreenOpen($sScreenName, $sDirectory, $sCocDiamond, $NeededRefs)
	...
	If $g_bDebugSetlog Then SetDebugLog("imgloc Searching for : " & $sScreenName & " in " & $sCocDiamond & " using " & $sDirectory, $COLOR_INFO)
	...
	;aux data
	Local $FoundRefs = 0
	If $g_bDebugSetlog Then SetDebugLog("imgloc MainScreen search Start", $COLOR_DEBUG)
	...
	If IsArray($result) Then ;We got return values
		For $fv = 0 To UBound($result) - 1
			...
			If $g_bDebugSetlog Then SetDebugLog("imgloc Found: " & $propsValues[0] & " at " & $propsValues[1], $COLOR_INFO)
	...
	Else
		;thnotfound
		If $g_bDebugSetlog Then SetDebugLog("imgloc Could not find " & $sScreenName & "!", $COLOR_WARNING)
		If $g_bDebugSetlog Then SetDebugLog("imgloc " & $sScreenName & " Calculated  (in " & Round(__TimerDiff($hTimer) / 1000, 2) & " seconds) :")
		Return False
	EndIf
...
Func isButtonVisible($sBtnName, $sbtnTile, $sBtnPlace)
	...
	If $g_bDebugSetlog Then SetDebugLog("isButtonVisible : looking for " & $sBtnName & " in " & $sBtnPlace & " with " & $sbtnTile, $COLOR_ORANGE)
	Local $result
...
Func imglocMainScreenReady()
	...
	If $ObjBtn = "" Then
		If $g_bDebugSetlog Then SetDebugLog("Main Screen is NOT Visible  : AttackBtn NOT VISIBLE!", $COLOR_ORANGE)
		$ReturnValue = False
		Return False
	EndIf
	...
	If $ObjBtn = "" Then
		If $g_bDebugSetlog Then SetDebugLog("Main Screen is NOT Visible  : OpenTrainBtn NOT VISIBLE!", $COLOR_ORANGE)
		$ReturnValue = False
		Return False
	EndIf
	$ObjBtn = isButtonVisible("ExpandChatBtn", @ScriptDir & "\imgxml\imglocbuttons\mainwindow\ExpandChat_0_91.xml", "0,320,40,440")
	If $ObjBtn = "" Then
		If $g_bDebugSetlog Then SetDebugLog("Main Screen is NOT Visible : ExpandChatBtn NOT VISIBLE!", $COLOR_ORANGE)
		$ReturnValue = False
		Return False
	EndIf
