...
Func isInsideBigDiamondXY($Coordx, $Coordy)

	Local $aCoords = [$Coordx, $Coordy]
	Return isInsideBigDiamond($aCoords)

EndFunc   ;==>isInsideDiamondXY

Func isInsideBigDiamond($aCoords)

	Local $Left = 20, $Right = 840, $Top = 10, $Bottom = 620 ; set the diamond shape
	...
	if($aCoords[0]<60) Then Return False
	if($aCoords[1]<60) Then Return False
	...
	If ($DX / $aSize[0] + $DY / $aSize[1] <= 1) Then
		...
		ElseIf $aCoords[0] > 692 And $aCoords[1] < 210 Then ; coordinates where the game will click on the GEMS button (safe margin)
			...
			Return False
		EndIf
		;If $debugSetlog = 1 Then Setlog("Coordinate Inside Village", $COLOR_GREEN)
		Return True ; Inside Village
	Else
		If $debugSetlog = 1 Then Setlog("Coordinate Outside Village", $COLOR_RED)
