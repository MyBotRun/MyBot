...
Func BuildingInfo($iXstart, $iYstart)
	...
	If StringInStr($sBldgText, "Tree") Then $sBldgText &= " (FakeLevel 101)"
	If StringInStr($sBldgText, "Mush") Then $sBldgText &= " (FakeLevel 100)"
	If StringInStr($sBldgText, "Trunk") Then $sBldgText &= " (FakeLevel 99)"
	If StringInStr($sBldgText, "Bush") Then $sBldgText &= " (FakeLevel 98)"
	If StringInStr($sBldgText, "Bark") Then $sBldgText &= " (FakeLevel 97)"
	If StringInStr($sBldgText, "Gem") Then
		If Not StringInStr($sBldgText, "Gem Mine") Then $sBldgText &= " (FakeLevel 96)"
	EndIf
	...
	If $aResult[2] > 95 Then
		If $aResult[2] = 200 Then
			$aResult[2] = "Broken" ; Broken Clan Castle/Clock Tower (not rebuild yet): report 'Broken' as Level
		Else
			$aResult[2] = ""
		EndIf
	EndIf
