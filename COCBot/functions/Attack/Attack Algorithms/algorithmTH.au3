...
Func AttackTHSmartBarch()
	local $count
		...
		SetLog("Attacking TH with 1st wave of BARCH")
		...
		SetLog("Attacking TH with 2nd wave of BARCH")

		 ; 2nd wave 25 secs total 16 barbs, 22 archers, works for TH partially covered by defenses
		AttackTHGrid($eBarb,4,2,2000,2,4,0) ; deploys 8 barbarians
		    If CheckOneStar() Then Return
		AttackTHGrid($eArch,2,5,2000,2,4,0) ; deploys 10 archers
		    If CheckOneStar() Then Return
		AttackTHGrid($eBarb,4,2,2000,2,4,0) ; deploys 8 barbarians
		    If CheckOneStar() Then Return
		AttackTHGrid($eArch,4,3,2000,2,4,0) ; deploys 12 archers
		...
		WEnd
		 ...
		 ;---3rd wave 26 secs (rather short interval until ALL IN) total 2 giants, 30 barbs, 57 archers
		SetLog("Oh! Seems like a trapped TH!")
		AttackTHGrid($eBarb,3,3,2000,3,4,0) ; deploys 9 barbarians
		    If CheckOneStar() Then Return
		AttackTHGrid($eArch,3,5,2000,3,4,0) ; deploys 15 archers
			If CheckOneStar() Then Return
		AttackTHGrid($eBarb,3,3,2000,3,4,0) ; deploys 9 barbarians
		    If CheckOneStar() Then Return
		AttackTHGrid($eArch,3,6,2000,3,4,0) ; deploys 18 archers
		    If CheckOneStar() Then Return
		AttackTHGrid($eBarb,4,3,2000,3,4,0) ; deploys 12 barbarians
		    If CheckOneStar() Then Return
		AttackTHGrid($eArch,4,6,2000,3,4,0) ; deploys 24 archers
		...
		Setlog("CHARGE!!!! ALL IN!")
		AttackTHGrid($eArch,2,10,1000,4,4,0) ; deploys 20 archers
			If CheckOneStar() Then Return
		AttackTHGrid($eBarb,2,10,1000,4,4,0) ; deploys 20 barbarians
			If CheckOneStar() Then Return		
		AttackTHGrid($eArch,2,10,1000,4,4,0) ; deploys 20 archers
			If CheckOneStar() Then Return
		AttackTHGrid($eBarb,2,10,1000,4,4,0) ; deploys 20 barbarians
		...
		AttackTHGrid($eBarb,2,10,1000,4,4,0) ; deploys 20 barbarians
			If CheckOneStar() Then Return
		AttackTHGrid($eArch,4,10,1000,4,4,0) ; deploys 40 archers
			If CheckOneStar() Then Return
		AttackTHGrid($eBarb,4,10,1000,4,4,0) ; deploys 40 barbarians
			If CheckOneStar() Then Return
		AttackTHGrid($eArch,4,10,1000,4,4,0) ; deploys 40 archers
			If CheckOneStar() Then Return
		AttackTHGrid($eBarb,4,10,1000,4,4,0) ; deploys 40 barbarians
			If CheckOneStar() Then Return	
		AttackTHGrid($eKing,1,1,1000,4,1,0) ; deploys king
			If CheckOneStar() Then Return
		AttackTHGrid($eQueen,1,1,1000,4,1,0) ; deploys queen
			If CheckOneStar() Then Return
		AttackTHGrid($eCastle,1,1,1000,4,1,0) ; deploys cc
			If CheckOneStar() Then Return

		SetLog("All troops deployed and waiting for a star...")
