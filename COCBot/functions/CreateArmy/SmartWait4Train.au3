...
Func SmartWait4Train($iTestSeconds = Default)
	...
	If ($g_bCloseWithoutShield Or BitAND($iTrainWaitCloseFlag, $TRAINWAIT_SHIELD) = $TRAINWAIT_SHIELD) And IsWaitforHeroesActive() Then
		...
		EndIf
		If $g_aiTimeTrain[2] > 0 Then $iTrainWaitCloseFlag = BitOR($iTrainWaitCloseFlag, $TRAINWAIT_HERO)
	...
	If ($iTrainWaitTime >= $MinimumTimeClose) Or $bTest Then ; are close times above minumum close time in GUI?
		If $iShieldTime > 0 Then ; is shield time positive?
			If $iDiffTime <= 0 Then ; is shield time less than total train time?
				; close game = $iShieldTime because less than train time remaining Or $iTrainWaitTime if $g_bCloseWithoutShield
				If $g_bCloseWithoutShield Then
				...
				EndIf
				UniversalCloseWaitOpenCoC(($g_bCloseWithoutShield ? $iTrainWaitTime : $iShieldTime) * 1000, "SmartWait4Train_", $StopEmulator, $bFullRestart, $bSuspendComputer)
	...
	Else
		...
		If ($iShieldTime < $iTrainWaitTime) And Not $g_bCloseWithoutShield Then ; only wait for lessor of shield time or training time due risk of app timeout with longer wait times
			If _SleepStatus($iShieldTime * 1000) Then Return
		Else
			If _SleepStatus($iTrainWaitTime * 1000) Then Return
		EndIf
		ResetTrainTimeArray()
	EndIf
